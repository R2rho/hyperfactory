...
const Panel = ({ children, style, className = "" }) => (
  <div
    className={`p-4 sm:p-6 md:p-8 lg:p-10 border border-border/50 bg-background/80 backdrop-blur-sm ${className}`}
    style={style}
  >
    {children}
  </div>
...
const H = ({ children, sub }) => (
  <div className="mb-4">
    <h2 className="text-3xl md:text-4xl font-bold tracking-tight font-alliance">{children}</h2>
    {sub && <p className="text-foreground/60 mt-2 max-w-3xl font-mono text-sm">{sub}</p>}
  </div>
);
...
const Bullet = ({ children }) => (
  <div className="flex items-start gap-3 text-base leading-7">
    ...
    <div className="text-foreground/85 font-mono text-sm">{children}</div>
  </div>
);
...
function HyperfactoryGeminiEffect() {
  ...
  return (
    <div className="relative w-full max-w-6xl mx-auto" ref={containerRef}>
      ...
      <div
        aria-hidden
        className="pointer-events-none absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-64 h-32 -z-10 opacity-15 rounded-full blur-3xl"
        style={{
        ...
        >
          <defs>
            ...
            <linearGradient id="geminiGradient1" x1="0%" y1="0%" x2="100%" y2="0%">
              ...
              <stop offset="50%" stopColor="rgba(255,199,0,0.4)" />
            ...
            <linearGradient id="geminiGradient3" x1="0%" y1="0%" x2="100%" y2="0%">
              ...
              <stop offset="50%" stopColor="rgba(255,199,0,0.2)" />
              <stop offset="100%" stopColor="rgba(255,199,0,0.1)" />
            </linearGradient>
...
function NodeCard({ title, subtitle, icon, tone = "gold" }) {
  const styles =
    tone === "gold"
      ? { background: "rgba(255, 199, 0, 0.05)", border: "1px solid rgba(255, 199, 0, 0.2)" }
      : { background: "rgba(255,255,255,0.03)", border: "1px solid rgba(255,255,255,0.1)" };
  return (
    <div className="relative p-4 border backdrop-blur-sm" style={styles}>
      <div className="flex items-center gap-3">
        <div className="h-9 w-9 grid place-items-center border border-primary/20 bg-primary/5">
          <span className="text-primary">{icon}</span>
        </div>
        <div>
          <div className="font-semibold text-white font-mono text-sm">{title}</div>
          <div className="text-xs text-white/60 font-mono">{subtitle}</div>
        </div>
      </div>
    </div>
  );
...
function CoreCard() {
  return (
    <div className="relative">
      <div
        className="relative p-6 border border-primary/30 bg-background/90 backdrop-blur-sm"
      >
        <div className="flex items-center gap-3">
          <div className="h-10 w-10 grid place-items-center border border-primary/30 bg-primary/10">
            <Cpu className="w-5 h-5 text-primary" />
          </div>
          <div>
            <div className="font-semibold text-white font-mono">Hyperfactory Backbone</div>
            <div className="text-xs text-white/60 font-mono">Orchestration · Unified Namespace · AI Agents</div>
          </div>
        </div>
...
const Chip = ({ children, icon }) => (
  <div className="flex items-center gap-2 px-2.5 py-2 border border-border/50 bg-background/50 font-mono">
    <span className="text-primary">{icon}</span>
...
const GeometricBackground = () => {
  ...
  useEffect(() => {
    ...
    const animate = () => {
      ...
      const primaryBright = 'rgba(255, 199, 0, 0.9)';
      const primaryMedium = 'rgba(255, 199, 0, 0.6)';
      const primaryDark = 'rgba(255, 199, 0, 0.3)';
      ...
      for (let row = 0; row < surfaceRows - 1; row++) {
        for (let col = 0; col < surfaceCols - 1; col++) {
          ...
          let surfaceColor;
          if (depth1 < 0.3) {
            surfaceColor = primaryBright;
          } else if (depth1 < 0.6) {
            surfaceColor = primaryMedium;
          } else {
...
const ValueCard = ({ icon, title, children }) => (
  <div className="p-5 border border-border/50 bg-background/80 backdrop-blur-sm">
    <div className="flex items-center gap-3 mb-2">
      <div className="h-9 w-9 grid place-items-center border border-primary/20 bg-primary/5">
        <span className="text-primary">{icon}</span>
      </div>
      <div className="text-xl font-semibold font-mono">{title}</div>
    </div>
    <div className="text-white/80 font-mono text-sm">{children}</div>
  </div>
);
...
const FeatureCard = ({ icon, title, description, highlight }) => (
  <div className="p-5 border border-border/50 bg-background/80 backdrop-blur-sm">
    <div className="flex items-start gap-4">
      <div className="h-10 w-10 grid place-items-center border border-primary/20 bg-primary/5 flex-shrink-0">
        <span className="text-primary">{icon}</span>
      </div>
      <div className="flex-1">
        <div className="text-lg font-semibold text-white mb-2 font-mono">{title}</div>
        {highlight && <div className="text-primary font-medium mb-2 font-mono">{highlight}</div>}
        <div className="text-white/80 text-sm leading-relaxed font-mono">{description}</div>
      </div>
    </div>
  </div>
);
        ...
        <Panel>
          <div className="flex flex-col sm:flex-row items-center gap-4 sm:gap-6">
            ...
            <div className="text-center sm:text-left">
              <h1 className="text-2xl sm:text-3xl md:text-4xl lg:text-5xl font-bold tracking-tight leading-tight font-alliance">Hyperfactory</h1>
              <p className="text-foreground/60 mt-1 sm:mt-2 text-sm sm:text-base font-mono">AI‑powered smart factory platform by Vertec</p>
            </div>
          </div>
        </Panel>
        ...
        <div className="flex flex-col items-center text-center">
          ...
          <Panel>
            <div className="max-w-4xl mx-auto">
              <h2 className="text-xl md:text-2xl font-medium leading-relaxed text-center font-alliance">
                Vertec is a manufacturing technology company on a mission to{" "}
                <span className="bg-gradient-to-b from-gray-200 via-gray-300 to-gray-500 bg-clip-text text-transparent font-semibold">revitalize American manufacturing</span>,{" "}
                <span className="bg-gradient-to-b from-gray-200 via-gray-300 to-gray-500 bg-clip-text text-transparent font-semibold">reindustrialize our production capacity</span>, and{" "}
                <span className="bg-gradient-to-b from-gray-200 via-gray-300 to-gray-500 bg-clip-text text-transparent font-semibold">uplift communities</span> to a better future.
              </h2>
            </div>
          </Panel>
        </div>
      ...
      <div className="space-y-6">
        ...
        <Panel>
          <div className="grid md:grid-cols-4 gap-4 text-center mb-6">
            <div>
              <div className="text-4xl md:text-5xl font-bold text-primary mb-2 font-alliance">98%</div>
              <div className="text-base md:text-lg text-white/80 mb-2 font-mono">of U.S. manufacturing are SMBs</div>
              <a href="https://www.score.org/resource/blog-post/how-small-manufacturing-businesses-drive-us-economy" target="_blank" rel="noopener noreferrer" className="text-xs text-primary/80 hover:text-primary underline font-mono">Source: SCORE</a>
            </div>
            <div>
              <div className="text-4xl md:text-5xl font-bold text-primary mb-2 font-alliance">75%</div>
              <div className="text-base md:text-lg text-white/80 mb-2 font-mono">employ fewer than 20 people</div>
              <a href="https://nam.org/mfgdata/" target="_blank" rel="noopener noreferrer" className="text-xs text-primary/80 hover:text-primary underline font-mono">Source: NAM</a>
            </div>
            <div>
              <div className="text-4xl md:text-5xl font-bold text-primary mb-2 font-alliance">89%</div>
              <div className="text-base md:text-lg text-white/80 mb-2 font-mono">can't fill job openings</div>
              <a href="https://www.deloitte.com/us/en/insights/industry/manufacturing-industrial-products/manufacturing-skills-gap-study.html" target="_blank" rel="noopener noreferrer" className="text-xs text-primary/80 hover:text-primary underline font-mono">Source: Deloitte</a>
            </div>
            <div>
              <div className="text-4xl md:text-5xl font-bold text-primary mb-2 font-alliance">42%</div>
              <div className="text-base md:text-lg text-white/80 mb-2 font-mono">use specialized software</div>
              <a href="https://itif.org/publications/2024/04/19/accelerating-digital-technology-adoption-among-smes/" target="_blank" rel="noopener noreferrer" className="text-xs text-primary/80 hover:text-primary underline font-mono">Source: ITIF</a>
            </div>
          </div>
        </Panel>
        ...
        <div className="space-y-6">
          ...
          <Panel>
            <div className="text-center mb-4">
              <h3 className="text-2xl font-bold text-primary font-alliance">Hyperfactory Advantage</h3>
            </div>
            <div className="grid md:grid-cols-4 gap-4">
              <div className="text-center">
                <Zap className="w-8 h-8 text-primary mx-auto mb-2" />
                <div className="font-semibold font-mono">Phased Approach</div>
                <div className="text-sm text-white/70 font-mono">90-day pilots</div>
              </div>
              <div className="text-center">
                <Cpu className="w-8 h-8 text-primary mx-auto mb-2" />
                <div className="font-semibold font-mono">AI-Guided</div>
                <div className="text-sm text-white/70 font-mono">Smart orchestration</div>
              </div>
              <div className="text-center">
                <Globe className="w-8 h-8 text-primary mx-auto mb-2" />
                <div className="font-semibold font-mono">Vendor-Neutral</div>
                <div className="text-sm text-white/70 font-mono">Open standards</div>
              </div>
              <div className="text-center">
                <Lock className="w-8 h-8 text-primary mx-auto mb-2" />
                <div className="font-semibold font-mono">Customer-Owned</div>
                <div className="text-sm text-white/70 font-mono">Your data, your control</div>
              </div>
            </div>
          </Panel>
        </div>
      ...
      <div className="space-y-6">
        ...
        <Panel>
          <div className="grid md:grid-cols-3 gap-6 text-center">
            <div>
              <Cpu className="w-8 h-8 text-primary mx-auto mb-2" />
              <div className="font-semibold font-mono">Platform IP</div>
              <div className="text-sm text-white/70 font-mono">Vertec proprietary</div>
            </div>
            <div>
              <Globe className="w-8 h-8 text-primary mx-auto mb-2" />
              <div className="font-semibold font-mono">Open Standards</div>
              <div className="text-sm text-white/70 font-mono">No vendor lock-in</div>
            </div>
            <div>
              <Lock className="w-8 h-8 text-primary mx-auto mb-2" />
              <div className="font-semibold font-mono">Customer Data</div>
              <div className="text-sm text-white/70 font-mono">100% customer owned</div>
            </div>
          </div>
        </Panel>
      </div>
    ...
    content: (
      <div className="space-y-8 relative">
        ...
        <div className="text-center space-y-8">
          <div className="space-y-6">
            <h2 className="text-5xl md:text-6xl font-bold bg-gradient-to-b from-gray-200 via-gray-300 to-gray-500 bg-clip-text text-transparent font-alliance">
              Thank You
            </h2>
            <p className="text-2xl md:text-3xl text-white/80 font-mono">
          ...
          <div className="space-y-4">
            <div className="flex items-center justify-center gap-3 text-primary">
              ...
              <a href="https://hyperfactory.com" target="_blank" rel="noopener noreferrer">
              <span className="text-3xl font-bold font-mono">hyperfactory.com</span>
              </a>
            </div>
          </div>
          ...
          <div className="pt-4 space-y-4">
            <div className="flex justify-center">
              <button
                className="px-8 py-4 bg-primary hover:bg-primary/80 text-black font-bold text-xl transition-colors duration-200 shadow-lg hover:shadow-xl border border-primary font-mono"
                onClick={() => window.open('mailto:info@vertec.io?subject=HyperFactory Presentation Follow-up', '_blank')}
              >
                Contact Us
              </button>
            </div>
...
export default function HyperfactorySlides() {
  ...
  return (
    <>
      ...
      {/* Top bar */}
      <header className="px-4 md:px-10 py-4 border-b border-border/50 flex items-center justify-between relative" style={{ zIndex: 10 }}>
        <div className="flex items-center gap-2 md:gap-3">
          <img src="/vertec.png" alt="Vertec" className="h-6 w-6 md:h-8 md:w-8 object-contain" />
          <span className="font-bold tracking-wide text-sm md:text-base font-mono">VERTEC / HYPERFACTORY</span>
          <span className="text-foreground/40 hidden md:inline text-sm font-mono">— slide {i + 1} of {slides.length}</span>
        </div>
        <div className="flex items-center gap-1 md:gap-2">
          <button
            onClick={prev}
            disabled={i === 0}
            className={`px-2 py-1 md:px-3 md:py-2 border transition flex items-center gap-1 md:gap-2 text-sm md:text-base font-mono ${
              i === 0
                ? 'bg-background border-border/50 text-foreground/30 cursor-not-allowed'
                : 'bg-background border-border/50 hover:border-primary/50 text-foreground'
            }`}
          >
          ...
          <button
            ...
            className="px-2 py-1 md:px-3 md:py-2 border border-primary bg-background hover:bg-primary/5 transition flex items-center gap-1 md:gap-2 text-sm md:text-base text-primary font-mono"
          >
            {i === slides.length - 1 ? (
      ...
      {/* Footer */}
      <footer className="px-6 md:px-10 py-4 border-t border-border/50 text-foreground/60 text-sm flex items-center justify-between font-mono">
